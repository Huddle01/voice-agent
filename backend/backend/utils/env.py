...
class EnvironmentVariables:
    ...
    def server_port(self) -> int:
        return self._env_vars['server_port']
...
def load_env_variables_declarative(env_spec) -> EnvironmentVariables:
    ...
    for var_key, var_config in env_spec.items():
        ...
        print(f"Loading environment variable '{env_name}'")

        env_value_str = os.getenv(env_name)

        print(f"Value: {env_value_str}")
        ...
        if expected_type is not None:
            try:
                if expected_type is bool:
                    env_value = env_value_str.lower() in ('true', '1', 'yes') if env_value_str else False
                ...
                raise TypeError(f"Environment variable '{env_name}' should be of type '{expected_type.__name__}', but got value '{env_value_str}' which cannot be converted.")
